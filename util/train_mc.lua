-- a more general version of train()
-- the model is the torch module and the dataset need to have two key 
-- `x` and `t` and have a size() function.
...
local function test(model, criterion, testset)
    ...
    -- loop testset
    for i = 1, testset:size(), 18 do
        local scores = torch.Tensor(18)
        local truth = 0
        for j = 0, 17 do
            local x = testset.x[i+j]
            local t = testset.t[i+j]
            local loss = criterion:forward(model:forward(x), t)
            total_loss = total_loss + loss
            scores[j+1] = model.output:float()
            if torch.eq(torch.Tensor{1}, t)[1] == 1 then
                truth = j+1
            end
        end
        local _, pred = torch.min(scores, 1)
        if pred[1] == truth then
            correct = correct + 1
        end
    end

    return total_loss/testset:size(), correct / (testset:size()/18)
end
...
function train(opt, model, criterion, trainset, testset)
    ...
    while true do
        ...
        -- some vars for computing the accuracy of trainset
        local scores = torch.Tensor(18)
        local si = 0 -- index of scores
        local truth = 0
        local correct = 0
        ...
        for i = 1, iters_per_epoch do
            ...
            if not opt.quiet and display_interval 
                ...
                local train_acc = correct / math.floor(i/18)
        ...
        if not opt.quiet then
            ...
            local train_acc = correct / (trainset:size()/18)
